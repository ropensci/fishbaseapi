...
class RemoteCommonParams < Minitest::Test

  $fields = "{\n  \"count\": 32792,\n  \"returned\": 1,\n  \"error\": null,\n  \"data\": [\n    {\n      \"SpecCode\": 16239,\n      \"Genus\": \"Aaptosyax\"\n    }\n  ]\n}"

  def test_fields
    res = fetch('species?limit=1&fields=SpecCode,Genus')
    assert res.ok?
  ...
  def test_offset_bad_character
    res = fetch('species?offset=adfadfads')
    assert !res.ok?
    assert 400, res.code
